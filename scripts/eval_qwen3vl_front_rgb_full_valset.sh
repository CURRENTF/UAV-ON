...
XOY_STEP_SIZE="${UAV_ON_XOY_STEP_SIZE:-3}"
Z_STEP_SIZE="${UAV_ON_Z_STEP_SIZE:-3}"
ROTATE_ANGLE="${UAV_ON_ROTATE_ANGLE:-90}"
...
if [[ -n "${QWEN3VL_MAX_NEW_TOKENS:-}" ]]; then
  MAX_NEW_TOKENS="$QWEN3VL_MAX_NEW_TOKENS"
elif [[ "$SAMPLE_MODE" == "trajectory" ]]; then
  MAX_NEW_TOKENS=128
else
  MAX_NEW_TOKENS=8
fi
