...
run_id="${UAV_ON_RUN_ID:-fourview_images_3m_20k_balanced_fast_approx}"
...
export UAV_ON_GENERATE_BASE_OUTPUT="${UAV_ON_GENERATE_BASE_OUTPUT:-/root/autodl-fs/datasets/uavon_qwen3vl_action_4view_images_3m_20k_balanced_fast_approx}"
...
export UAV_ON_COLLECTION_SCENE_BALANCE_MODE="${UAV_ON_COLLECTION_SCENE_BALANCE_MODE:-per_scene_shards_2000_each}"
...
export UAV_ON_FRONT_VIEW_ONLY="${UAV_ON_FRONT_VIEW_ONLY:-0}"
export UAV_ON_SEPARATE_VIEW_IMAGES="${UAV_ON_SEPARATE_VIEW_IMAGES:-1}"
...
export UAV_ON_IMAGE_SETTLE_SECONDS="${UAV_ON_IMAGE_SETTLE_SECONDS:-0.2}"
export UAV_ON_SET_POSE_SETTLE_SECONDS="${UAV_ON_SET_POSE_SETTLE_SECONDS:-0.2}"
...
export UAV_ON_COLLECTION_VARIANT="${UAV_ON_COLLECTION_VARIANT:-4view_images_3m_20k_balanced_fast_approx}"
export UAV_ON_COLLECTION_TAGS="${UAV_ON_COLLECTION_TAGS:-3m,4view,separate-images,rgb-only,20k,balanced-scenes,fast,approx,non-strict}"
export UAV_ON_COLLECTION_NOTE="${UAV_ON_COLLECTION_NOTE:-Movement labels use approximate/non-strict 3m A* units via astar_voxel_resolution=3.0 and astar_max_move_voxels=1; rotation labels remain degrees. Each sample stores four separate RGB images in front,left,right,down order instead of a stitched grid.}"
...
echo "[run] separate_view_images=${UAV_ON_SEPARATE_VIEW_IMAGES}" | tee -a "$generator_log"
